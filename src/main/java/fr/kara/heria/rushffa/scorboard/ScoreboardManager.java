...
public class ScoreboardManager {
    ...
    public ScoreboardManager() {
        ...
        glowingTask = RushFFA.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            String ip = colorIpAt();
            for (PersonalScoreboard scoreboard : scoreboards.values())
                RushFFA.getInstance().getExecutorMonoThread().execute(() -> scoreboard.setLines(ip));
        }, 80, 80, TimeUnit.MILLISECONDS);

        reloadingTask = RushFFA.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            for (PersonalScoreboard scoreboard : scoreboards.values())
                RushFFA.getInstance().getExecutorMonoThread().execute(scoreboard::reloadData);
